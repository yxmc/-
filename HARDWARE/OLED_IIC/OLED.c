...
void OLED_W_SCL(uint8_t BitValue)
{
	...
	GPIO_WriteBit(OLED_SCL, OLED_SCL_IO, (BitAction)BitValue);
...
void OLED_W_SDA(uint8_t BitValue)
{
	...
	GPIO_WriteBit(OLED_SDA, OLED_SDA_IO, (BitAction)BitValue);
...
void OLED_GPIO_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = OLED_SCL_IO;
	GPIO_Init(OLED_SCL, &GPIO_InitStructure);
	GPIO_InitStructure.GPIO_Pin = OLED_SDA_IO;
	GPIO_Init(OLED_SDA, &GPIO_InitStructure);
...
void OLED_WriteData(uint8_t* Data, uint8_t Count)
{
	...
	// for (i = 0; i < Count; i++)
	// {
	// 	I2C_SendData(I2C1, Data[i]);	//依次发送Data的每一个数据
	// }
